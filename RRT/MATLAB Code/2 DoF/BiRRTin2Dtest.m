...
map = imbinarize(imread('map2.bmp'));
imshow(map);
hold on;
q_start = [10;10];
q_goal = [490;490];
...
for iter = 1:imax
    if size(ST1,2) >= size(ST2,2)
        [ST2, ST1, isReach, collichecktime, q2end, q1end] = ExtendTree(ST2, ST1, q_start, p_g, map, error, dim, L,collichecktime);
        if isReach == 1, break; end
    else
        [ST1, ST2, isReach, collichecktime, q1end, q2end] = ExtendTree(ST1, ST2, q_goal, p_g, map, error, dim, L,collichecktime);
        if isReach == 1, break; end
    end
end
...
function [EST, OST, isReach, collichecktime, q1end, q2end] = ExtendTree(EST, OST, q, p_g, map, error, dim, L,collichecktime)
    isReach = 0;
    q1end = []; q2end = [];
    if rand < p_g
...
%             pause(0.2)
            ...
            if dist <= error
                plot([q_othernear(1) q_new(1)],[q_othernear(2) q_new(2)],'color','#D95319','LineWidth',2);
                pause(0.2);
...
%             pause(0.2);
    %       pause(0.1)
            ...
            if norm(q_goal - q_new) < error
                 plot([q_goal(1) q_new(1)],[q_goal(2) q_new(2)],'color','#77AC30','LineWidth',2);
%                 pause(0.2);
                qreturn = q_new;
                isReach = 1;
                break;
